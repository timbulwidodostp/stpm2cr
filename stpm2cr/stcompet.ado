*! stcompet 1.0.4 EC - MB 2 FEB 2004                           (SJ4-2: st0059)
...
******************************************************************************
	...
	syntax /* ... */  [if] [in] , COMPET1(numlist missingokay) [ COMPET2(numlist  missingokay) /*
		...
		*/  COMPET6(numlist missingokay) BY(varname) Level(integer $S_level) ]
	marksample touse
...
*** 1 - Further checks
	...
	while "`compet`i''" != "" {
		local c = `i' + 1
		while "`compet`c''" != "" {
			cap bothlist `compet`c'' \ `compet`i''
			if "`r(list)'" != "" {	
				di as err "You specify `r(list)' as codes for two competing events"
                  	exit 459
        		}
			local c = `c' + 1 
		}
...
***     main event and in competing options order
	...
	quietly {
		...
		if "`by'" != "" { 
			tempvar byvar
			egen `byvar' = group(`by')
                        levels7 `byvar' if `touse'  /* byvar values */ 
...
* High bound as in Marubini & Valsecchi - arcsin limits look useless
*			gen `hi' = exp(log(`cuminc')+`lvl'*`sevar'/`cuminc')  

			label var `hi' "CI() `level'% lower bound"
      		}
		if "`lo'" != "" {
			gen `lo' = `cuminc' ^ exp(-`lvl'*`sevar'/(`cuminc' *log(`cuminc' )))  

* Low bound as in Marubini & Valsecchi  
*			gen `lo' = exp(log(`cuminc')-`lvl'*`sevar'/`cuminc')  
