...
program Estimate, eclass byable(recall)
	...
	foreach n in `causelist' {													
		...
		if "`dftvc_c`n''" != "" & "`knotstvc_c`n''" != "" {
			display as error "Only one of dftvc() or knotstvc() can be specified in equation `n'"
			exit 198
		}		
		...
		if "`tvc_c`n''" != "" {
			if "`dftvc_c`n''" == "" & "`knotstvc_c`n''" == "" {
				display as error "The dftvc or knotstvc option is compulsory if you use the tvc option in equation `n'"
				exit 198
			}
			if "`knotstvc'" == "" {
        ...
        if "`knotstvc_c`n''" != "" {
                if "`dftvc_c`n''" != "" {
                        display as error "You can not specify both the dftvc and knotstvc options together"
                        exit 198
                }
		...
		if "`bknots_c`n''" != "" {
                tokenize `bknots_c`n''
				local lowerknot_c`n' `1'
                local upperknot_c`n' `2'
        }
		...
		if `nbhknots' == 0 & "`rcsbaseoff_c`n''" == "" & "`cure_c`n''" == "" { 
			qui rcsgen `lnt' if `touse', df(`df_c`n'') gen(_rcs_c`n'_) dgen(_d_rcs_c`n'_) if2(`_d`n'') bknots(`bknots')  `orthog' `reverse_c`n''
			local rknots_c`n' `r(knots)'
		}
		else if `nbhknots' > 0 & "`rcsbaseoff_c`n''" == "" & "`cure_c`n''" == "" {
			local allknots_c`n' `lowerknot_c`n'' `knots_c`n'' `upperknot_c`n''

			/*forvalues i=1/`nbhknots' {
					if substr("`knscale_c`n''",1,1) == "t" {
							local addknot = ln(real(word("`knots'",`i')))
					}
					local allknots_c`n' `allknots_c`n'' `addknot'
			}
			local allknots_c`n' `allknots_c`n'' `upperknot'*/

			qui rcsgen `lnt' if `touse', knots(`allknots_c`n'') gen(_rcs_c`n'_) dgen(_d_rcs_c`n'_) if2(`_d`n'')  `orthog' `reverse_c`n''
			local rknots_c`n' `r(knots)'
			//tokenize `knots_c`n''
			//local df : word count `knots_c`n''
			//local df = `df' - 1
			//di `df'
		...
		if `nbhknots' == 0 & "`rcsbaseoff_c`n''" == "" & "`cure_c`n''" != ""{
			...
			qui rcsgen `lnt' if `touse', df(``df_c`n''_temp') gen(`rcs_`n'_') dgen(`d_rcs_`n'_') if2(`_d`n'') bknots(`bknots')  `orthog' `reverse_c`n''
			...
			foreach tvcvar in  `tvc_c`n'' {
				...
				if "`rcsbaseoff_c`n''" == "" & "`cure_c`n''" == ""{
					if "`tvcknots_`tvcvar'_user_c`n''" == "" {
						qui rcsgen `lnt' if `touse', df(`tvc_`tvcvar'_df_c`n'') gen(_rcs_`tvcvar'_c`n'_) dgen(_d_rcs_`tvcvar'_c`n'_) if2(`_d`n'') bknots(`bknotstvc_c`n'') `orthog' `reverse_c`n''
						local rknotstvc_`tvcvar'_c`n' `r(knots)'
					}
					else {
						local tvcknots_`tvcvar'_user_c`n' `lowerknot_`tvcvar'_c`n'' `tvcknots_`tvcvar'_user_c`n'' `upperknot_`tvcvar'_c`n''
						di "`tvcknots_`tvcvar'_user_c`n''"
						qui rcsgen `lnt' if `touse', knots(`tvcknots_`tvcvar'_user_c`n'') gen(_rcs_`tvcvar'_c`n'_) dgen(_d_rcs_`tvcvar'_c`n'_) if2(`_d`n'') `orthog' `reverse_c`n''
						local rknotstvc_`tvcvar'_c`n' `r(knots)'
					...
					if `nbhknots' == 0 {
						...
						qui rcsgen `lnt' if `touse', df(``tvc_`tvcvar'_df_c`n''_temp') gen(`rcs_`tvcvar'_`n'_') dgen(`d_rcs_`tvcvar'_`n'_') if2(`_d`n'') bknots(`bknotstvc_c`n'')  `orthog' `reverse_c`n''
						local rknotstvc_`tvcvar'_c`n' `r(knots)'
...
//di "delayed entry `n': `conslist'"
	...
	*/
		...
		`captureml' ml model `mlmethod' stpm2cr_ml_`scale'() /// 
			`mleqfit' ///
			if `touse' ///
			`wt', ///
			`mlopts' ///
	...
	*/	
		...
		`captureml' ml model `mlmethod' stpm2cr_ml`addlf'_`scale' /// 
			`mleqfit'  ///
			if `touse' ///
			`wt', ///
			`mlopts' ///
	...
	foreach n in `causelist' {
        ...
        foreach tvcvar in  `tvc_c`n'' {
                local exp_lowerknot_c`n' = exp(`lowerknot_`tvcvar'_c`n'')
                local exp_upperknot_c`n' = exp(`upperknot_`tvcvar'_c`n'')
                ereturn local boundary_knots_`tvcvar'_c`n' "`exp_lowerknot_c`n'' `exp_upperknot_c`n''"
        }
        if `df_c`n'' >1 {
